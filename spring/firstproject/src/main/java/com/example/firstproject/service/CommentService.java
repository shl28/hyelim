...
//        return commentRepository.findByArticleId(articleId).stream().map(CommentDto::createCommentDto).toList();
        ...
        return commentRepository.findByArticleId(articleId)  // commentRepository 목록을 조회
                .stream() // stream 으로 변경
                .map(comment -> CommentDto.createCommentDto(comment))  // createCommentDto 통해 comment(entity)를 하나씩 전달하여 dto로 변환
    ...
    public CommentDto update(Long id, CommentDto dto) {
        // 댓글 조회 및 예외 발생
        Comment target = commentRepository.findById(id).orElseThrow(()->new IllegalArgumentException("댓글 수정 실패! 대상 댓글 없습니다."));
    ...
    public CommentDto delete(Long id) {
        // 댓글 조회 및 예외 발생
        Comment target = commentRepository.findById(id).orElseThrow(()->new IllegalArgumentException("댓글 삭제 실패! 대상 댓글 없습니다."));
