addEventListener('DOMContentLoaded', ()=>{
    ...
    const artItems = document.querySelectorAll('article');

    window.addEventListener('scroll', ()=>{
        const sct = window.scrollY || window.pageYOffset;

        artItems.forEach((artItem, i)=>{
            artItem.style.transform = `translateZ(${(-5000 * i) + sct}px)`;
        
            if(sct >= 5000 * i && sct < 5000 * (i +1)) {
                artItems.forEach(el => el.classList.remove('on'));
                navItems.forEach(el => el.classList.remove('on'));
                artItem.classList.add('on');
